...
v0.8.0 于 2026-07-15 封盘。该版本不改变 GameData schema、builder、检索排序或源数据，
继续复用 schema 2 中文 GameData DB，主要交付移动端证据检查与 Summary / Fact-check 交互统一。

主要交付：

- Summary 增加取消、重试、错误/空状态与迟到响应保护，并与 Fact-check 统一来源栏和停止交互。
- `search_local_lore` observation 通过正式 parser 转换为可展开 GameData 证据卡，展示 title、
  section、content type、source path、raw id、retrieval type、ranking reason、trust note 和原文。
- 覆盖度仅区分 direct candidate 与 retrieved context，不把 retrieval score 表述为事实确定性。
- Agent 加载、推理、空、错误和取消文案完成中英文本地化；窄屏与 2x 文字缩放纳入 Widget 测试。
- 未引入 Wiki seed、Book indexing、embedding、vector、TFLite 或新的 Agent evidence source。
...
- `ArkLores-0.8.0.apk`：27206050 bytes，SHA256
  `f246e55b53690f3139a29d82d4f8c2d1abfecb59068bf5d204b67785f752c5e3`
- `arklores_gamedata_zh.db`：395022336 bytes，SHA256
  `3b6d61417da8bed1edda4535762d20f5ca2135389a37df3fa2c9fc4f785ab7c0`
- `arklores_gamedata_zh.db.gz`：115092521 bytes，SHA256
  `8870945a23e399b00736fff77883db8b1e4bd8eec866d9395aa0841ff01aabd5`
...
- `/home/hhikr/flutter/bin/flutter test`：58 passed，3 个 opt-in 外部 Chat QA skipped。
- `/home/hhikr/flutter/bin/flutter analyze`：No issues found。
- finalized 完整 DB retrieval QA passed：11 个固定 query、3 个 `特蕾西娅` alias candidates
  和 `activities/act21mini/level_act21mini_st07.txt:3` scoped evidence。
- release-mode APK 注入 v0.8.0 GameData asset URL 与 SHA256，versionName `0.8.0`、versionCode `8`。
- `tools/setup.sh` release dry-run passed；`apksigner` v1/v2 verified，签名证书为 Android Debug。

`ArkLores-0.8.0.apk` 是 GitHub release-mode 验收包，不是正式商店签名包。

封盘时 deferred：

- Android 真机上的长回答、横屏、双语、极端文字缩放与 TalkBack 验收。
- v0.8 真实外部 Chat 专项矩阵。
- `source_path` 到原始 GameData 文件的应用内导航。
- 正式商店签名。
