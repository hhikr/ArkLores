...
#
# 用法:
#   ./tool/setup.sh                     # 进入交互模式（默认）
#   ./tool/setup.sh --interactive       # 强制进入交互模式
#   ./tool/setup.sh --help              # 查看命令行帮助
#   ./tool/setup.sh --action uninstall,build,install --platform android --mode debug
#
set -euo pipefail
...
示例:
  ./tool/setup.sh                           # 交互式菜单运行
  ./tool/setup.sh -a build -p android       # 仅构建 Android 包
  ./tool/setup.sh -a uninstall,install      # 卸载并重新安装（使用已有包）
  ./tool/setup.sh -p ios -t simulator       # iOS 模拟器构建与部署
EOF
  exit 0
}
  ...
  if [ "$PLATFORM" = "android" ]; then
    ...
    if [ "$DEPLOY_TARGET" = "simulator" ]; then
      ...
      if [ "$BUILD_MODE" = "debug" ]; then
        ...
        "$FLUTTER" build ios --debug --no-codesign || "$FLUTTER" build ios --debug
      else
